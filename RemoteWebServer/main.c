...
static void show_usage_and_exit(void) {
  ...
  fprintf(stderr, "Dr. House's Remote Web Server %s (c) Dr. House, built %s\n",
          mg_version(), __DATE__);
...
static void init_server_name(void) {
  snprintf(server_name, sizeof(server_name), "Dr. House's Remote Web Server v. %s",
           mg_version());
}
    ...
    case WM_USER:
      switch (lParam) {
        ...
        case WM_LBUTTONDBLCLK:
          ...
          AppendMenu(hMenu, MF_STRING | (!service_installed ? MF_GRAYED : 0),
                     ID_REMOVE_SERVICE, "Uninstall service");
          AppendMenu(hMenu, MF_SEPARATOR, ID_SEPARATOR, "");
